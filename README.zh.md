...
点左侧 ribbon 的轨道图标，或运行命令 "Open galaxy view"。左上角的控制面板最上面是**「过滤」**，往下是预设选择器、可折叠的「外观 / 深空背景 / 辉光 / 力学」分区、导航与动效区（自动环绕、漫游、连接两篇）以及高级选项。悬停任意滑杆并双击可重置；一切自动持久化。
...
- **悬停 →「只看」**＝只留这一个文件夹；再点还原。
- **「全部显示」**（分区标题右侧）一键还原。
- 未解析／孤儿／标签三个开关也在这里——它们回答的是同一个问题：什么进图。

颜色与图里完全一致（含你从 2D 图谱导入的配色组），所以这个分区同时就是配色图例。
