...
- #6 在本机 1.12.7 的 Minimal（bloom=0、starfield=off、空间背景全关）下未复现，3230 节点仍正常；发布后仍等 reporter 补其 Windows / 主题环境的 0.6.0 复现矩阵与录屏。PR #8 在替代实现发布前保持开放。
- 当前进入 0.6.0 发布链路：先由发布分支通过完整门禁并合入 `main`，再从远端 `main` 推送 tag；不会用仅有本地构建或仅推 tag 代替 Release 验收。
