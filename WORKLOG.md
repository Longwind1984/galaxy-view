...
## 2026-07-22 · 开放问题依次修复：弹出窗口时钟、Canvas 与 Tag Lens

### 做了什么

按现有 issues / PR 的风险顺序完成三项本地修复，并对第四项保留证据门槛。弹出窗口中的相机飞行、巡游停留和开场镜头不再混用不同窗口的时间原点，rAF 与可见性也能原子交接到弹出窗口；Markdown 与 Canvas 共用图谱入口，Canvas 的 `file` 卡片会成为「画布 → 库文件」链接；原 PR #8 没有硬 rebase，而是复用现有 metadata 入口，完整落地独立「标签」分区、单选 Tag Lens、主标签配色与 5–50 有界标签枢纽。交叉审查另修复质量档只降低 WebGLRenderer DPR、却未同步 EffectComposer DPR 的独立性能债。#6 没有猜测性改渲染器，而是在真实 Obsidian 中补做「无辉光 + 无背景」A/B；图仍正常渲染。

自动门禁之后又在 Obsidian 1.12.7、3230 篇真实开发库中完成实机验收：Galaxy 标签移入独立 `about:blank` 窗口并最大化后，「回中心」仍保持 3230 节点、约 60 fps；标签 Lens、配色和 20 个 hub 都能实际显示，hub 上限从 20 调到 5 时节点数精确从 3250 变为 3235。实机验收同时发现自定义 Slider 只能鼠标操作，随后补成可 Tab 聚焦的标准 ARIA slider，并验证方向键 20→21→20。

### 关键决策与被否决的备选

- **#14 同时修时间域与窗口所有权**：跨 Electron 窗口的 rAF timestamp 不保证共享 time origin；相机 tween / path / idle、巡游 dwell 和 establishing shot 改为消费不截短的安全 elapsed，WASD 与渲染仍保留 100ms 防跳上限。每个待执行 rAF 记录创建它的 Window，换窗立即在旧 owner 取消并向新 owner 交接；旧 rAF 与旧 IntersectionObserver 的迟到回调用 generation／identity 丢弃，dispose 后不能复活。被否决：等待旧窗口“再跑一帧”才换 owner，或只 `disconnect()` observer 却接收已排队旧回调。
- **#15 用实机证据推翻最初假设后，补最小 Canvas 解析层**：Obsidian 1.13.1 探针明确返回 `resolvedLinks[canvasPath] = null` 且 `getFileCache(canvas) = null`，所以只改 `getFiles()` 会得到孤立 Canvas 节点。最终只解析 JSON Canvas 的 `file` 卡片，不解释文本卡、网页卡或画布内部 edge；启动读取按 8 文件限并发，之后按文件防抖增量更新，revision 防止旧异步读取覆盖新内容，临时坏 JSON 保留最后有效缓存，文件夹改名/删除会失效整棵子树。被否决：继续依赖不存在的核心 Canvas 元数据，或另建一套完整 Canvas 图模型。
- **#7 保留 PR #8 的产品范围、重切数据边界**：维护者已公开要求保留 Lens、标签配色和有上限 hub，完成审计时据此否决了早先 Lens-only 的缩窄实现。最终只沿现有 `showTags` 路径读取一次 metadata，笔记直接携带 tags；Lens/chips/主标签稳定 hash 配色都读这份数据，只有 `showTags && showTagHubs` 时才生成 top-N 枢纽与边。滑杆与构建端双重限制 5–50、180ms 防抖，关闭 hubs 时零隐藏节点／力学成本。被否决：复制 PR 的第二套标签提取、继续使用与背景层冲突的 nebula 命名、或机械 rebase 旧 23 文件 diff。
- **#6 不把猜测写成修复**：0.6.0 候选仍保持 `alpha:false`、默认 `preserveDrawingBuffer:false`、不透明 scene background，并且 RenderPass 每帧清目标。原报告在 0.2.2 同时关闭 bloom 与 starfield，只能证明合成负载可能放大问题，不能证明 bloom 是根因。被否决：直接发布 `powerPreference:'high-performance'`、按主题名猜透明主题、或显式重复默认的 `preserveDrawingBuffer:false`。
- **实机验收发现的 Slider 缺口当场修，而不是用鼠标可用掩盖**：原控件是纯 `div`，Tab 会直接跳过。轨道现有 `role=slider`、完整 aria value、Tab 焦点、方向键与 Home/End；键盘、拖动和双击复位统一走同一更新路径。无新增依赖，辅助功能树已实际识别为 slider。

### 当前状态：现在能跑什么、怎么跑

- 最终自动验证：`npm test` 为 16 个测试文件、126 项全部通过；`npm run build` 通过；`npm run lint` 为 0 error、2 条仓库既有 warning；`git diff --check` 通过。
- `npm test`：覆盖双 Window rAF owner 交接、旧 observer 回调、dispose 防复活、相机 / 巡游、Markdown / Canvas 文件入口与边、过滤、Tag Lens／颜色／hubs 生命周期和旧设置迁移。
- `npm run build`：类型检查并生成 `dist/` 插件产物。
- `npm run lint`：检查 TypeScript、测试和 Obsidian 商店规则。
- `npm run dev`：构建到隔离的 `dev-vault/`，用于最后的 Obsidian 实机交互与视觉确认。
- Obsidian 1.13.1 实机探针：核心 Canvas 元数据为空；最终 GraphStore 仍实际生成 `Canvas Smoke.canvas → Canvas Source.md` 与 `→ Canvas Target.md` 两条边。探针、夹具与临时诊断插件随后全部删除，开发插件清单恢复。
- Obsidian 1.12.7 实机 UI：默认深空场 3230n/19337l、约 60 fps；极简预设明确显示星空关闭、辉光强度 0.00，图仍正常；弹出独立窗口最大化后回中心正常；Tag Lens、主标签着色、5/20 hub 上限与 Slider 键盘路径均通过。
- 0.6.0 已发布：[PR #17](https://github.com/Longwind1984/galaxy-view/pull/17) squash 合入 `main` 的 `e21c0ca` 后才推 tag；[Release 工作流](https://github.com/Longwind1984/galaxy-view/actions/runs/29932194187) 成功，[Release](https://github.com/Longwind1984/galaxy-view/releases/tag/0.6.0) 为正式版本。公开 `main.js`、`manifest.json`、`styles.css` 与本地最终构建逐字节一致。

### 未尽事项与已知问题

- #14 已在 macOS + Obsidian 1.12.7 走完「移到弹出窗口 → 最大化 → 回中心」完整路径；仍需 Windows 11 做平台验收。自动测试另覆盖负 delta、换窗口首帧与完成进度边界。
- 标签分区已用本机 Chrome 在 252px 面板上实际渲染深色／浅色的 Lens + 配色 + 5-hub 激活态；据图修复了浅色分区标题与激活 chip 计数对比度，两个开关、滑杆、reset 和 12 chips 均未溢出。Rick 已确认真实 Obsidian 截图并授权发布。
- #6 在本机 1.12.7 的 Minimal（bloom=0、starfield=off、空间背景全关）下未复现，3230 节点仍正常；仍等 reporter 补其 Windows / 主题环境的 0.6.0 复现矩阵与录屏。
- #7 与 #15 已由 PR #17 自动关闭；#14 保持开放等待 Windows 11 复验。PR #8 仍开放，待另发“替代实现已随 0.6.0 发布”的贡献者收尾说明；PR #16 不属于本版，需独立审查性能与大库表现。

### 文件级变更清单

- 文档与版本：`README.md`、`README.zh.md`、`docs/社区巡检.md`、`WORKLOG.md`、`package.json`、`package-lock.json`、`manifest.json`、`versions.json`
- 时间域与窗口：`src/timing/{frameClock,windowFrameLoop,windowVisibility}.ts`、`src/interactions/CameraDirector.ts`、`src/tour/TourDirector.ts`、`src/view/GraphController.ts`
...
- 标签探索：`src/types.ts`、`src/data/{tagLens,buildGraph,GraphStore}.ts`、`src/render/palette.ts`、`src/overlay/{ControlPanel,Slider}.ts`、`src/settings.ts`、`styles.css`、`src/i18n/{de,en,es,it,pt,zh}.ts`、`src/view/{GraphController,SearchModal}.ts`
- 性能：`src/quality/tiers.ts`、`src/render/AggregateRenderer.ts`
- 可视草案：`demo/tag-lens-demo.html`
