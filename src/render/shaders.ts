...
attribute vec3 aTagColor;
attribute float aTagStrength;
attribute float aTagHub;
varying vec3 vColor;
varying vec3 vTagColor;
varying float vGhost;
varying float vDim;
varying float vTagStrength;
varying float vTagHub;
...
void main() {
	...
	float core = smoothstep(0.20, 0.0, d) * mix(0.55, 0.78, vTagHub) * (1.0 - vGhost) * (1.0 - uLightMode);
	...
	float tagRing = smoothstep(0.30, 0.39, d) * smoothstep(0.50, 0.42, d) * vTagStrength;
	float hubRing = smoothstep(0.18, 0.30, d) * smoothstep(0.48, 0.36, d) * vTagHub;
	tagRing = max(tagRing, hubRing);
	col = mix(col, vTagColor, tagRing * 0.85);

	float alpha = max(smoothstep(0.5, 0.42, d) * mix(1.0, 0.45, vGhost), tagRing * 0.95) * vDim;
